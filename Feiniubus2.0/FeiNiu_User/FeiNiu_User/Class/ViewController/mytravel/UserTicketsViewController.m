...
- (void)viewDidLoad {
    ...
    [self performSelector:@selector(scrolltoSelect) withObject:nil afterDelay:.4];
}
...
{
    if (_selcetIndex != 0) {
        [self.collectionView scrollToItemAtIndexPath:[NSIndexPath indexPathForRow:_selcetIndex inSection:0] atScrollPosition:(UICollectionViewScrollPositionCenteredHorizontally) animated:YES];
    }
}
...
#pragma mark - Public
- (void)setTickets:(NSArray<Ticket *> *)tickets{
    self.ivEmpty.hidden = tickets.count != 0;
   _tickets = [tickets sortedArrayUsingComparator:^NSComparisonResult(Ticket * _Nonnull obj1, Ticket * _Nonnull obj2) {
        if (obj1.ticketState <= EmTicketStatus_NotTake && obj2.ticketState <= EmTicketStatus_NotTake) {
            return NSOrderedSame;
        }else if (obj1.ticketState == EmTicketStatus_NotTake){
            return NSOrderedAscending;
        }else if (obj2.ticketState == EmTicketStatus_NotTake){
            return NSOrderedDescending;
        }else if (obj1.ticketState == EmTicketStatus_HasTake && obj2.ticketState == EmTicketStatus_HasTake){
            return NSOrderedSame;
        }else if (obj1.ticketState == EmTicketStatus_HasTake){
            return NSOrderedAscending;
        }else if (obj2.ticketState == EmTicketStatus_HasTake){
            return NSOrderedDescending;
        }else{
            return NSOrderedSame;
        }
...
- (void)showInViewController:(UIViewController *)vc{
    ...
    self.view.backgroundColor = [UIColor colorWithRed:.3 green:.3 blue:.3 alpha:.7];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (ticket.ticketState <= EmTicketStatus_NotTake) {
        cell.ivUsedIcon.image = [UIImage imageNamed:@"ticket_not_get"];
    }else if (ticket.ticketState == EmTicketStatus_HasTake){

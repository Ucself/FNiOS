...
- (void)viewDidLoad {
    ...
    //self.navTitle.text = _vcTitle;
    //self.navView.title = _vcTitle;
    [self initWebView];
...
//    [MBProgressHUD showHUDAddedTo:self.view animated:YES];
    if (self.urlString && ![self.urlString isKindOfClass:[NSNull class]] && self.urlString.length != 0) {
        [self startWait];
...
- (void)webViewDidFinishLoad:(UIWebView *)webView{
    [self stopWait];
    self.navView.title = [webView stringByEvaluatingJavaScriptFromString:@"document.title"];
//    [MBProgressHUD hideHUDForView:self.view animated:YES];
}

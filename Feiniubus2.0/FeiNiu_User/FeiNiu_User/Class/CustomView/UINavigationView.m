...
-(void)initUI
{
    ...
    _labelTitle.textColor = UITextColor_Black;
    ...
    [self setBackgroundColor:UINavigationBKColor];
...
- (void)layoutSubviews
{
    ...
    _btnLeft.frame = CGRectMake(0, 27, 46, 30);
    
    _btnRight.frame = CGRectMake(self.bounds.size.width-rightBtnWidth-12, 27, rightBtnWidth, 30);
...
{
    if ([self.delegate respondsToSelector:@selector(navigationViewBackClick)]) {
        [self.delegate navigationViewBackClick];
    }
}

-(void)btnRightClick:(id)sender
{
    if ([self.delegate respondsToSelector:@selector(navigationViewRightClick)]) {
        [self.delegate navigationViewRightClick];
    }
}

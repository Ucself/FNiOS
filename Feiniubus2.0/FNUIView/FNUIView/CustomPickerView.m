...
-(void)iniUI
{
    ...
    _contentView.backgroundColor = UIColorFromRGB(0xF4F4F8);
    ...
    [bgView makeConstraints:^(MASConstraintMaker *make) {
        make.height.equalTo(kContentHeight - 40);
...
-(void)btnOKClick
{
    ...
    //闭包回调
    if (_clickComplete) {
        _clickComplete(_dataSourceArray[self.selectedIndex]);
    }
    //闭包回调
    if (_clickCompleteIndex) {
        _clickCompleteIndex(self.selectedIndex,self.useType);
    }
    //取消视图
    [self cancelPicker:self.parentView];
}

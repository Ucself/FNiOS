...
@property (nonatomic) int offsetMargin;  //本项目添加
...
- (void)setUtilityButtons:(NSArray *)utilityButtons parentCell:(SWTableViewCell *)parentCell utilityButtonSelector:(SEL)utilityButtonSelector;
...
- (void)populateUtilityButtons {
    ...
    for (UIButton *utilityButton in _utilityButtons) {
        ...
        [utilityButton setFrame:CGRectMake(utilityButtonXCord, _offsetMargin, _utilityButtonWidth, CGRectGetHeight(self.bounds)- _offsetMargin)];
...
- (void)initializer {
    ...
    scrollViewContentView.backgroundColor = UIColor_Background;
    ...
    if (![NSStringFromClass([[subs objectAtIndex:0] class]) isEqualToString:kTableViewCellContentView]) {
        // iOS 7
        contentViewParent = [subs objectAtIndex:0];
    }
...
-(void)setOffsetMargin:(int)offsetMargin
{
    //本项目添加
    _scrollViewButtonViewLeft.offsetMargin = offsetMargin;
    _scrollViewButtonViewRight.offsetMargin = offsetMargin;
}

- (void)setButtonsLeft:(NSArray*)left right:(NSArray*)right
{
    self.rightUtilityButtons = right;
    self.leftUtilityButtons = left;
    [self setupButtons];
}

-(void)setupButtons
{
    [self.scrollViewButtonViewLeft setUtilityButtons:self.leftUtilityButtons parentCell:self utilityButtonSelector:@selector(leftUtilityButtonHandler:)];
    [self.scrollViewButtonViewLeft setFrame:CGRectMake([self leftUtilityButtonsWidth], 0, [self leftUtilityButtonsWidth], _height)];
    
    [self.scrollViewButtonViewRight setUtilityButtons:self.rightUtilityButtons parentCell:self utilityButtonSelector:@selector(rightUtilityButtonHandler:)];
    [self.scrollViewButtonViewRight setFrame:CGRectMake(CGRectGetWidth(self.bounds), 0, [self rightUtilityButtonsWidth], _height)];
    
    [self.scrollViewButtonViewLeft populateUtilityButtons];

...
- (void)show
{
    ...
    int heigth = (int)self.items.count * KRowHeight;
    self.tableView.frame = CGRectMake(0, self.arrowSize.height, 118, heigth);
...
- (void)layoutSubviews
{
    ...
    frame.size.height = (int)self.items.count * KRowHeight -1;
    
    CGFloat frameMidx = self.arrowShowPoint.x-CGRectGetWidth(frame)*0.5 - 35;
...
-(void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath
{
    if ([cell respondsToSelector:@selector(setSeparatorInset:)]) {
        [cell setSeparatorInset:UIEdgeInsetsMake(0, 45, 0, 17)];
    }

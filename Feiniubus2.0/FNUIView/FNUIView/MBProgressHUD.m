...
+ (void)showTip:(NSString *)tipMsg WithType:(FNTipType)type withDelay:(NSTimeInterval)delay{
        ...
        switch (type) {
            case FNTipTypeFailure:{
                tipImage = [UIImage imageNamed:@"icon_failure"];
                break;
            }
            case FNTipTypeSuccess:{
                tipImage = [UIImage imageNamed:@"icon_rights"];
                break;
            }
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
		...
		self.minSize = CGSizeMake(120, 80);
		self.square = NO;
...
- (void)updateIndicators {
	...
	if (mode == MBProgressHUDModeIndeterminate &&  !isActivityIndicator) {
        ...
        [(UIActivityIndicatorView *)indicator setTransform: transform];

//
//  CarOwnerForgotPasswordViewController.m
//  FeiNiu_CarOwner
//
...
#import "CarOwnerForgotPasswordViewController.h"
...
@interface CarOwnerForgotPasswordViewController ()<UITextFieldDelegate,DynaButtonDelegate>
{
    BOOL isPasswordType;
}
...
@implementation CarOwnerForgotPasswordViewController
...
- (IBAction)registClick:(id)sender {
    ...
    [tempDic setObject:_phoneNumberTextField.text forKey:@"phone"];
    [tempDic setObject:_codeTextField.text forKey:@"code"];
    [tempDic setObject:[_passwordTextField.text md5] forKey:@"password"];
    [tempDic setObject:[[PushConfiguration sharedInstance] getRegistrationID] forKey:@"registrationId"];
    [tempDic setObject:[[NSNumber alloc] initWithInt:2] forKey:@"terminalType"];
    [tempDic setObject:[[NSNumber alloc] initWithInt:2] forKey:@"vType"];
    [tempDic setObject:[[NSNumber alloc] initWithInt:EMUserRole_CarOwner] forKey:@"userRole"];
    
    [self startWait];
    [[ProtocolCarOwner sharedInstance] putInforWithNSDictionary:tempDic urlSuffix:KUrl_Password requestType:KRequestType_Password];
}
...
-(void)httpRequestFinished:(NSNotification *)notification
{
    ...
    switch (resultParse.requestType) {
        ...
        {
            //获取验证码的返回
            if (resultParse.resultCode == 100006) {
                [self showTipsView:@"手机号码不正确"];
            }
        }
            break;
        case KRequestType_Password:
        {

...
-(void)setTagAndAlias:(NSString*)tag alias:(NSString*)alias userId:(NSString*)userId
{
    ...
    lastTimeTag = tag;
    lastUserId = userId;
    [APService setTags:tags alias:aliasString callbackSelector:@selector(tagsAliasCallback:tags:alias:) object:self];
}
//清除标签
-(void)resetTag
{
    //记录一次
    lastTimeTag = @"";
    [APService setTags:[NSSet new] callbackSelector:@selector(tagsAliasCallback:tags:alias:) object:self];
}
//清除别名
-(void)resetAlias
{
...
-(void)resetTagAndAlias
{
    ...
    lastTimeTag = @"";
    lastTimeAlias = @"";
    [APService setTags:[NSSet new] alias:@"" callbackSelector:@selector(tagsAliasCallback:tags:alias:) object:self];
}
                    ...
                    alias:(NSString*)alias
{
    ...
    //如果注册失败等待一段时间再执行
    if (iResCode !=0) {
        [self performSelector:@selector(setTagAndAlias) withObject:nil afterDelay:120];
    }
}
//注册失败的时候重新发送
-(void)setTagAndAlias
{
    ...
    [APService setTags:tags alias:lastTimeAlias callbackSelector:@selector(tagsAliasCallback:tags:alias:) object:self];
}

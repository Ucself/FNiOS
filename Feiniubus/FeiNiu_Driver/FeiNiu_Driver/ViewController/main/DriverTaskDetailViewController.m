...
@interface DriverTaskDetailViewController ()<UITableViewDelegate,UITableViewDataSource,MAMapViewDelegate>
{
...
@implementation DriverTaskDetailViewController
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    UIViewController *tempController = segue.destinationViewController;
    if ([tempController isKindOfClass:[DriverPassengerListViewController class]]) {
        ((DriverPassengerListViewController*)tempController).driverTaskModel = _taskModel;
    }
}
...
-(void)startLocationUpload
{
    ...
    tempDriverLocationUploadModel.taskType = @"拼车";
    tempDriverLocationUploadModel.orderId = _taskModel.scheduleId;
    tempDriverLocationUploadModel.licensePlate = _taskModel.vehicleLicensePlate;
...
-(void)initInterface
{
    ...
    else if (_taskModel.orderStatus == 1)
    {
        //设置按钮背景色
        ...
        [_submitButton setBackgroundColor:UIColorFromRGB(0xB4B4B4)];
        [_submitButton setTitle:@"等待开始" forState:UIControlStateNormal];
    }
}
...
-(void)jumpScanPassenger:(id)sender
{
    ...
    CHScanViewController *scanPassengerViewController = [storyboard instantiateViewControllerWithIdentifier:@"DriverScanPassengerId"];
    scanPassengerViewController.driverTaskModel = _taskModel;
    ...
    switch (alertView.tag) {
        case 1001:
        {
            ...
            if (buttonIndex == 1) {
                //点击结束任务
                [self startWait];
                [[ProtocolDriver sharedInstance] putInforWithNSDictionary:[NSDictionary new] urlSuffix:[[NSString alloc] initWithFormat:@"%@?scheduleId=%@&status=%d&action=%d",Kurl_carpoolOrderDriverTask,_taskModel.scheduleId,1,2] requestType:KRequestType_driverStartTask];
            }
        ...
        case 1002:
        {
            ...
            if (buttonIndex == 1) {
                [self startWait];
                //点击结束任务
                [[ProtocolDriver sharedInstance] putInforWithNSDictionary:[NSDictionary new] urlSuffix:[[NSString alloc] initWithFormat:@"%@?scheduleId=%@&status=%d&action=%d",Kurl_carpoolOrderDriverTask,_taskModel.scheduleId,2,2] requestType:KRequestType_driverEndTask];
            }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (indexPath.section) {
        case 0:
        {
            ...
            //验票
            UIView *ticketView = [tempCell viewWithTag:101];
            [ticketView setUserInteractionEnabled:YES];
            UITapGestureRecognizer *ticketTapGestureRecognizer = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(jumpScanPassenger:)];
            [ticketView addGestureRecognizer:ticketTapGestureRecognizer];
            //乘客明细 toPassengerList
            UIView *numberView = [tempCell viewWithTag:102];
            [numberView setUserInteractionEnabled:YES];
            UITapGestureRecognizer *numberTapGestureRecognizer = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(jumpPassengerDetail:)];
            [numberView addGestureRecognizer:numberTapGestureRecognizer];
            
            //显示的数据信息
            //任务时间
            UILabel *timeLabel = (UILabel*)[tempCell viewWithTag:111];
            timeLabel.text = [[NSString alloc] initWithFormat:@"%@",_taskModel.startingDate];
            //车辆信息
            UILabel *vehicleInforLabel = (UILabel*)[tempCell viewWithTag:112];
            vehicleInforLabel.text = [[NSString alloc] initWithFormat:@"%@%@  准乘坐%d人 1辆",_taskModel.vehicleTypeName,_taskModel.vehicleLevelName,_taskModel.vehicleSteats];
            //订单类型
            UILabel *orderTypeLabel = (UILabel*)[tempCell viewWithTag:113];
            orderTypeLabel.text = @"拼车";
            //车辆牌照
            UILabel *licensePlateLabel = (UILabel*)[tempCell viewWithTag:114];
            licensePlateLabel.text = [[NSString alloc] initWithFormat:@"%@",_taskModel.vehicleLicensePlate];
            //乘坐人数
            UILabel *seatsLabel = (UILabel*)[tempCell viewWithTag:115];
            seatsLabel.text = [[NSString alloc] initWithFormat:@"%d",_taskModel.peopleAmount];
            //起始地
            UILabel *startNameLabel = (UILabel*)[tempCell viewWithTag:116];
            startNameLabel.text = [[NSString alloc] initWithFormat:@"%@",_taskModel.startingName];
            //目的地
        ...
        case 1:
        {
            ...
            if (_taskModel.orderStatus >=3)
            {
                ...
                [requestDic setObject:_taskModel.scheduleId forKey:@"ScheduleId"];
                [[ProtocolDriver sharedInstance] getLocationWithNSDictionary:requestDic urlSuffix:Kurl_CarpoolLocation requestType:KRequestType_getCarpoolLocation];
            }
            else
            {
                //未完成的任务展示位置
...
-(void)httpRequestFinished:(NSNotification *)notification
{
    ...
    switch (resultParse.requestType) {
        ...
        case KRequestType_driverEndTask:
        {
            ...
            break;
        case KRequestType_getCarpoolLocation:
        {
            if (resultParse.resultCode == 0) {
                //根据返回的路径绘制路径
